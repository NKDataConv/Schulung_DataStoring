...
# 1. die Datenbank Verbindung für jeden Request geöffnet und nicht geschlossen
# 2. Es findet keine Validierung der Daten statt (siehe post Methode)
...
def insert_data(data: dict):
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO weather (Date, Location, MinTemp) VALUES (?, ?, ?)", (data["Date"], data["Location"], data["MinTemp"]))
    conn.commit()
    conn.close()
